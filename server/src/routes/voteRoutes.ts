  ...
  async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      if (!voterId) {
        return res
          .status(400)
          .json({ error: "voterId (matric number) is required" });
      }
      ...
      if (!event) {
        return res
          .status(400)
          .json({ error: "Voting is currently not allowed." });
      }
      const candidate = event.candidates.id(candidateId);
      if (!candidate) {
        return res
          .status(404)
          .json({ error: "Candidate not found in the event" });
      }
      ...
      if (existingVote) {
        return res
          .status(400)
          .json({ error: "You have already voted in this Event." });
      }
      ...
      if (!updatedEvent) {
        return res
          .status(500)
          .json({ error: "Error updating candidate votes" });
      }
      ...
      if (!updatedCandidate) {
        return res
          .status(500)
          .json({ error: "Candidate not found in the event after update" });
      }
