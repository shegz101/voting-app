...
import express from "express";
import VotingEvent from "../models/VotingEvent";
// import Student from "../models/Student"; // Uncomment if you need to verify student identity

const router = express.Router();

// GET /api/students/voting-events
// Retrieves all voting events with status "ongoing"
router.get("/voting-events", async (req, res) => {
  try {
    // Optionally, if you want to verify that the user is a student, you might get userId from req.body or headers
    // const { userId } = req.body;
    // const student = await Student.findById(userId);
    ...
    console.error("Error fetching ongoing voting events:", error);
    res.status(500).json({ error: "Internal server error" });
  }
});

// GET /api/students/voting-events/:eventId/candidates
// Retrieves the list of candidates embedded in the specified voting event
router.get("/voting-events/:eventId/candidates", async (req, res) => {
  try {
    const { eventId } = req.params;
    const event = await VotingEvent.findById(eventId);
    if (!event) {
...
router.get("/voting-events/:eventId/results", async (req, res) => {
  try {
    ...
    // If you want to restrict access to results, check a flag like event.isResultsPublic
    // if (!event.isResultsPublic) {
    //   return res.status(403).json({ error: "Results are not yet available" });
    // }
    // Return the candidates with basic info (name and votes)
    const results = event.candidates.map(candidate => ({
      name: candidate.name,
      votes: candidate.votes,
    }));
